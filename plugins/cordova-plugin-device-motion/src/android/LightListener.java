...
public class LightListener extends CordovaPlugin implements SensorEventListener {
    ...
    private float x;                                // most recent acceleration values
    ...
    private int start() {
        ...
        if ((list != null) && (list.size() > 0)) {
          ...
          if (this.sensorManager.registerListener(this, this.mSensor, SensorManager.SENSOR_DELAY_UI)) {
              ...
              this.fail(LightListener.ERROR_FAILED_TO_START, "Device sensor returned an error.");
              return this.status;
          };

        } else {
          this.setStatus(LightListener.ERROR_FAILED_TO_START);
          this.fail(LightListener.ERROR_FAILED_TO_START, "No sensors found to register ambient light listening to.");
          return this.status;
        }
